...
设计结论：用工类型不新增物理字段，复用 `outsourced_staff.employment_category`。人员新增、更新和离场均按 `personCode` 匹配 `outsourced_staff.id`，待入场新增时同时将 `personCode` 写入 `outsourced_staff.account_id`。必须先完成存量数据及码表口径切换、再启用 org 消息监听，避免新旧 `6` 混存而无法区分。自动新增、自动更新及自动离场均以消息写入后的 `employment_category` 是否为 `6` 判断。

数据展示结论：内包页面仅展示 org 同步的 `employment_category=6` 人员，账户字段展示其内部账号（`account_id`）；原外包页面仅展示外包账号，查询时排除 `employment_category=6` 的内包人员。内、外包页面的数据范围相互隔离；对外 RPC 不透出内包账户数据。

## 3. 消息消费与字段落库

### 3.1 消息入口

运营支撑在人员新增或修改时向以下 SOFAMQ Topic 广播 `PersonMessageModel` JSON；HRMS 新增一个消费者订阅该消息。消费者必须监听该 Topic 的全部人员新增/修改消息，不按用工类型在 MQ 订阅层过滤；收到消息后先按 `personCode` 查询 `outsourced_staff.id`，再依据消息用工类型、HR 在岗状态、组织是否变化、原用工类型和 HRMS 当前状态决定新增、更新、离场或忽略。
...
人员 MQ 表示 HR 已离职，或用工类型由 `6-新劳务规则` 变为非 `6` 时，HRMS 先查询人员当前数据。仅原用工类型为 `6` 且当前为已入场状态（`staff_status=0`）的人员执行自动离场；人员不存在、处于待入场/已离场状态或原用工类型非 `6` 时自动忽略，不重复结束任职或释放项目编制。
...
自动离场必须按目标状态幂等处理：同一人员重复收到离职或转出用工类型消息时，已离场记录直接忽略，不重复写任职记录、不重复执行账号失效，也不重复写项目编制流水。内包账户的自动离场结果仅保存在 HRMS 本地，不发送现有外包人员变更 MQ。
...
| `personEmploymentType`      | `String`     | `employment_category` | 是         | 顶层字段为权威值；切换后 `6` 表示新劳务规则，`7` 表示原有劳务外包。                  |
...
| `personEmploymentSource`    | `String`     | -                     | 否         | 本期不覆盖 `source_from`；人员来源由 HRMS 本次消息消费场景统一管理。            |
...
本次对接资料的 JSON 顶层用工类型字段为 `personEmploymentType`，未出现顶层 `employmentType`。HRMS 消费时以顶层 `personEmploymentType` 为权威值，不从 `personExtendInfo` 内嵌字符串取值；“6-新劳务规则”的准确码值及码表上线顺序仍需运营支撑确认。

## 4. 数据改动

### 4.1 数据库

用工类型和人员匹配均不新增物理字段；内部工号复用 `outsourced_staff.id`，内部账号复用 `outsourced_staff.account_id`。上线前须先完成码表和存量数据迁移，再部署或启用消息消费者，顺序不可颠倒：
...
**Description：** 根据内包页面查询条件分页查询内部转外包人员，仅返回 `employment_category=6` 的内包账户。账户名称为内部账号（新一代工号），在本接口中复用请求及响应字段 `accountId`，实际查询和返回值均取 `outsourced_staff.account_id`。

**OpenAPI：** `POST /platform/api/aboss/hrms/staff/query-internal-outsourced-staff-page`

**复用类型：**

- 请求：`QueryStaffPageCmd`
- 响应元素：`QueryStaffPageDTO`
- 返回：`PageResult<QueryStaffPageDTO>`

原接口 `POST /platform/api/aboss/hrms/staff/query-staff-page` 保持不变。本接口只复用 DTO 结构，不复用原查询入口和 Repository 查询方法。
...
| `staffStatusList`   | `List<string>` | 在职状态集合：`-1` 待入场、`0` 已入场、`1` 已离场               | false    | -     |
| `permissionOrgCode` | `string`       | 当前页面机构权限编码，沿用现有机构权限校验                         | true     | -     |

复用 DTO 中未传的请求字段保持为空，按现有查询逻辑不参与过滤，无需额外处理。请求中不增加 `employmentCategory`；内包范围必须由后端固定控制。
...
归属组织范围由三个请求字段共同决定：`permissionOrgCode` 表示右上角当前切换的权限机构，`operatingOrgCode` 表示查询区域手动选择的归属组织，`includeChildren` 表示是否包含所选组织的下级机构。页面初始化时 `operatingOrgCode` 为空；手动选择后传所选组织编码。

| 当前切换机构          | 页面初始化                                   | 手动选择归属组织且未勾选“包含下级” | 手动选择归属组织且勾选“包含下级”                    |
| --------------- | --------------------------------------- | ------------------ | ------------------------------------ |
| 总公司“中华财险”       | 查询中华财险本级及全部下级机构，即全公司数据                  | 仅查询所选组织本级          | 查询所选组织本级及全部下级机构                      |
| 分公司，例如浙江分公司     | 查询当前分公司本级及全部下级机构                        | 仅查询所选组织本级          | 查询所选组织本级及全部下级机构                      |
| 中心支公司，例如杭州中心支公司 | 查询当前中心支公司和对应上级分公司，即杭州中心支公司、浙江分公司两个机构的数据 | 仅查询所选组织本级          | 中支场景不使用“包含下级”；即使传入 `true`，也按所选组织本级查询 |
|                 |                                         |                    |                                      |

结合现有代码，处理规则如下：

1. 总公司初始化：`permissionOrgCode` 为 `HrmsConstant.PARENT_ORG_CODE` 且 `operatingOrgCode` 为空。现有 `OutsourcedStaffPermissionHelper#getOperatingOrgCodeList` 返回空集合，Repository 不追加机构 `IN` 条件，等价于查询中华财险本级及全部下级机构。
2. 分公司初始化：`operatingOrgCode` 为空。沿用现有 `OrgBasicMapper#selectChildrenOrgByCode(permissionOrgCode)`，根据 `branch_org_manage_path` 查询当前分公司本级及全部下级机构编码。
3. 总公司或分公司手动选择归属组织：未勾选“包含下级”时，机构集合只放入 `operatingOrgCode`；勾选后调用 `selectChildrenOrgByCode(operatingOrgCode)`，获取所选组织本级及全部下级机构编码。
4. 中支初始化：需求要求查询“当前中支 + 对应上级分公司”。现有 `getOperatingOrgCodeList` 标注“中支不上翻”，只会调用 `selectChildrenOrgByCode(permissionOrgCode)`，不能返回上级分公司，因此本接口需要新增中支初始化分支：根据当前中支通过现有组织集成获取 `OrgBranchInfoDTO.branchCompanyOrgCode`，机构集合只放入当前 `permissionOrgCode` 和对应 `branchCompanyOrgCode`。
5. 中支手动选择归属组织：机构集合固定只放入 `operatingOrgCode`，不向下展开。后端不能只依赖前端隐藏或取消“包含下级”，在中支权限机构场景下即使收到 `includeChildren=true` 也必须按本级查询。

最终组织编码集合用于过滤 `outsourced_staff.operating_org_code`。后端仍需结合当前登录人的真实机构权限校验请求参数，不能因为前端传入其他 `permissionOrgCode` 或 `operatingOrgCode` 而扩大查询范围。
...
**Description：** 内包账户修改页提交接口，仅允许修改 `outsourced_staff.employment_category='6'` 的人员。页面中的人员基本信息、岗位信息、在职状态、入离场日期和任职记录均为只读信息，不由本接口提交；本接口只接收人员内部工号、关联项目、关联协议和当前权限机构。待入场人员提交成功后按 3.1.3 直接入场，已入场人员提交成功后仅调整项目、协议及其带出的业务条线、供应商信息。

**OpenAPI：** `POST /platform/api/aboss/hrms/staff/update-internal-outsourced-staff`

**接口类型：**

- 请求：新增 `UpdateInternalOutsourcedStaffCmd`
- 返回：`Boolean`

现有接口 `POST /platform/api/aboss/hrms/staff/update-staff` 保持不变，本接口不复用该接口的 `UpdateStaffCmd` 和保存方法。现有 `UpdateStaffCmd` 继承 `CreateStaffCmd`，包含证件、学历、政治面貌、照片等本页面不维护的必填字段；现有保存方法还会在已入场状态下调用账号中心并整条重建人员记录，不符合内包页面只修改项目、协议的范围。
...
| `projectCode`       | `string` | 关联项目编码                          | true     | -     |
| `agreementNo`       | `string` | 关联协议编码                          | true     | -     |
| `permissionOrgCode` | `string` | 当前页面右上角切换的权限机构编码，用于校验项目、协议可操作范围 | true     | -     |

`businessLineCode` 和 `supplierId` 不由前端传入：后端分别根据 `projectCode`、`agreementNo` 查询并写入，避免前端篡改。`accountId`、`displayName`、`phone`、`employmentCategory`、组织、岗位、在职状态、入离场日期等只读字段均不在本接口请求 DTO 中定义。
...
curl -X POST \
  -H 'Content-Type: application/json; charset=utf-8' \
  -i '/platform/api/aboss/hrms/staff/update-internal-outsourced-staff' \
  --data '{
    "staffId": "1000123400",
...
1. 后端按 `staffId` 查询 `outsourced_staff.id`，并强制校验 `employment_category='6'`。人员不存在或用工类型不是内包账户时拒绝修改，不能通过直接调用接口修改普通外包人员。
2. 仅 `staff_status=-1`（待入场）和 `staff_status=0`（已入场）允许提交；`staff_status=1`（已离场）拒绝修改。后端以数据库当前状态为准，不接收前端传入的在职状态。
3. 关联项目、协议必须存在且处于可关联状态。根据 `permissionOrgCode` 和当前登录人真实权限校验可选范围：中支可选择所属分公司和本中支的项目、协议；分公司可选择本分公司的项目、协议。后端必须执行同样校验，不能只依赖前端下拉框过滤。
4. 后端根据项目带出 `business_line_code`，根据协议带出 `supplier_id`。项目、协议、业务条线和供应商作为一个事务整体保存，任一步骤失败均不产生部分更新。
5. 当本次操作需要新占用一个项目编制时，保存前计算目标项目实际剩余数量；实际剩余数量小于等于 `0` 时拒绝保存，提示“项目计划额度不足，请先申请计划数量”。已入场人员未更换项目、仅修改协议时不重复占用编制，也不因当前项目剩余数量为 `0` 阻断保存。
6. 待入场人员提交成功后，在同一事务内写入项目、协议、业务条线、供应商，写系统来源负数编制流水占用一个项目编制，并按 3.1.3 将 `staff_status` 从 `-1` 更新为 `0`、写入入场信息和任职记录；不发起审批，不调用账号中心创建或恢复账号，`id` 和 `account_id` 保持不变。
7. 已入场人员保持 `staff_status=0` 和原入场日期不变。项目未变化时不写编制流水；项目发生变化时，同一事务内向原项目写正数流水释放一个编制、向新项目写负数流水占用一个编制，再更新项目、协议、业务条线、供应商及任职记录。
8. 内包账户的修改和直接入场结果仅保存在 HRMS 本地，不发送现有外包人员变更 MQ。

#### Response-parameters

| Parameter | Type | Description |
| --- | --- | --- |
| `data` | `boolean` | `true` 表示修改成功；校验或保存失败时按统一异常结构返回失败原因 |
...
| 场景 | 处理 |
| --- | --- |
| `staffId`、`projectCode`、`agreementNo` 或 `permissionOrgCode` 为空 | 参数校验失败，不执行保存 |
| 人员不存在或 `employment_category` 不为 `6` | 拒绝修改 |
| 人员已离场 | 拒绝修改 |
| 项目或协议不存在、不可关联或超出当前登录人权限 | 拒绝修改并返回对应原因 |
| 目标项目需要新增占用但实际剩余数量小于等于 `0` | 返回“项目计划额度不足，请先申请计划数量” |
| 人员状态或项目编制在提交期间发生变化 | 事务回滚，前端刷新页面后重新提交 |

### 5.3 查询内包账户详情

**Description：** 内包详情页面专用查询接口，根据人员内部工号一次返回基本信息、岗位信息、工作信息和任职记录分页数据。接口只允许查询 `outsourced_staff.employment_category='6'` 且当前登录人具有数据权限的人员；普通外包人员即使使用合法 `staffId` 直接请求也不返回。
...
- 岗位元素：新增 `InternalOutsourcedStaffPositionDTO`
- 任职记录分页：复用 `PageResult<QueryStaffDetailDTO>` 的分页及元素字段结构

现有 `POST /platform/api/aboss/hrms/staff/query-staff-detail` 和 `POST /platform/api/aboss/hrms/staff/query-staff-movement-page` 保持不变。本接口可复用两者的 Repository 查询与名称补全能力，但不能直接复用原 Controller 入口：现有详情查询未强制过滤 `employment_category=6`，也没有内包页面的数据权限校验；现有任职查询还需要前端再次请求，不能一次返回需求 2.5 的完整页面数据。
...
1. 先按 `staffId` 查询 `outsourced_staff.id`，同时强制追加 `employment_category='6'`；未命中时按未查询到内包账户处理。
2. 机构和业务条线数据权限沿用 5.1 的内包账户查询规则。后端根据当前登录人真实权限、`permissionOrgCode`、人员所属机构及已关联业务条线执行校验，不能因为前端从列表页携带了 `staffId` 就跳过权限判断。
3. 人员主数据由 `outsourced_staff` 返回；账户取 `account_id`，人员工号取 `id`。组织名称沿用现有组织集成补全，项目和业务条线名称沿用 `ProjectService` 补全，协议和供应商名称沿用 `AgreementService` 补全。
4. 任职记录按 `outsourced_staff_movement.staff_id=staffId` 查询，并按 `gmt_create` 倒序分页；响应中的 `gmtModified` 使用该条任职记录的 `gmt_create`，与现有任职分页查询保持一致。没有任职记录时返回总数 `0` 和空数组。
5. 当前代码只能根据 `outsourced_staff.work_position_code` 和组织集成构造零条或一条岗位数据。接口响应按需求定义为 `positionList`，以便后续支持多岗位；“主岗位/兼职岗位”及多岗位数据必须在第 6 章待确认项第 9 条明确数据来源后才能完整赋值。
6. 本接口为只读查询，不修改人员、项目、协议、编制或任职记录，也不发送 MQ。
...
| `operatingOrgCode` | `string` | 所属经营组织编码 |
| `operatingOrgName` | `string` | 所属经营组织名称 |
| `positionList` | `List<InternalOutsourcedStaffPositionDTO>` | 岗位信息列表；无数据时返回空数组 |
| `positionList[].workPositionCode` | `string` | 工作岗位编码 |
| `positionList[].workPositionName` | `string` | 工作岗位名称 |
| `positionList[].workPositionType` | `string` | 工作岗位类型，展示“主岗位”或“兼职岗位”；数据来源待确认 |
...
| 人员不存在或 `employment_category` 不为 `6` | 按未查询到内包账户处理              |
...
1. 后端对 `staffIds` 去重并校验原集合不存在重复值，再按 ID 集合批量查询 `outsourced_staff`。查询必须强制追加 `employment_category='6'`，且查询结果数量必须与请求人数一致；任一人员不存在或不是内包账户时整批拒绝。
...
1. 对接资料已确认顶层 `personEmploymentType` 为用工类型字段；仍需确认“6-新劳务规则”的实际码值和码表变更上线顺序。
