...
1. **适用范围与选择规则**
   ...
   - 项目、协议的可选范围沿用当前登录人权限：中支可选择分公司和中支的项目、协议；分公司可选择分公司的项目、协议。
   - 选择项目后，前端根据项目所属业务条线筛选可选协议。项目和协议不要求属于同一机构，也不存在预先绑定关系，例如可选择杭州中支的项目与浙江分公司的协议。

2. **后端提交校验**

   后端按 `staffIds`、`projectCode`、`agreementNo` 和当前登录人权限，依次校验人员范围、项目和协议状态、项目计划额度，以及项目与协议的业务条线是否一致。前端筛选仅用于展示，后端必须再次校验业务条线。
