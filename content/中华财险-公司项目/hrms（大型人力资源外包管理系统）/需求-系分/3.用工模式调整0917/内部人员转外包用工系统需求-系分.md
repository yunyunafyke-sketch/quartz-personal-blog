...
2. **提交与校验顺序**
   ...
   用户在内包账户列表勾选人员，并选择同一项目、协议后提交。后端按本次提交的 `staffIds`、`projectCode`、`agreementNo` 和当前登录人权限依次校验人员、项目和协议；校验完成后，统一为勾选人员关联项目、协议、业务条线和供应商，并更新任职记录。

3. **项目计划额度校验**

   后端以本次勾选人数与项目实际剩余数量进行校验：

   - 勾选人数 ≤ 项目的实际剩余数量时，可以继续提交关联。
   - 勾选人数 ＞ 项目的实际剩余数量时，不可以关联，并提示：

     > 项目计划额度不足，实际剩余数量XX，当前选择人员数量XX，请先申请计划数量

4. **人员处理结果**

   - 在职状态为“待入场”的人员：成功关联项目和协议后，状态变更为“已入场”，并更新任职记录。
   - 在职状态为“已入场”的人员：在职状态保持不变，更新关联项目和协议，并更新任职记录表。

5. **批量提交结果**

   本接口以本次勾选人员为一个批次处理。仅当人员、项目、协议和项目计划额度均校验通过后，才执行本批次关联更新；提交成功时返回 `true`。
