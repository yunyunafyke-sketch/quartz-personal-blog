...
本图覆盖从 SOFAMQ 消费者收到消息到提交消费确认的完整处理过程。消费者订阅人员广播消息的全部内容；业务层仅在消息用工类型为 `6`，或 HRMS 已有记录且原用工类型为 `6` 时，进入 ITO 分支处理。

颜色按处理阶段和业务分支分类：<span style="color:#2563EB">■ 蓝色：监听、解析、校验与条件判断</span>　<span style="color:#D97706">■ 橙色：当前用工类型为 `6` 的在岗/实习处理</span>　<span style="color:#7C3AED">■ 紫色：由 `6` 变为其他类型</span>　<span style="color:#16A34A">■ 绿色：忽略、删除或消费确认</span>　<span style="color:#374151">■ 深灰色：离职/退休处理</span>　<span style="color:#DC2626">■ 红色：解析或校验异常</span>。

```mermaid
flowchart TB
    A[SOFAMQ 人员广播消息<br/>Topic 和 Tag] --> B[消费者收到消息<br/>读取消息体与消息 ID]
    B --> C[JSON 反序列化为<br/>PersonMessageModel]
    C --> D{对象是否为空}
    D -->|是| E[记录业务异常日志]
    D -->|否| F[Bean 参数校验]
    F --> G{参数校验是否通过}
    ...
    G -->|是| H[记录接收日志]
    H --> I[标准化入职、离职、出生日期]
    I --> J[按 personCode 查询<br/>HRMS 人员快照]

    J --> K{当前用工类型<br/>是否为 6}
    K -->|否| L{存在人员快照且<br/>原用工类型为 6}
    K -->|是| M{人员状态是否为<br/>离职、退休、实习结束}
    L -->|否| N[非 ITO 链路<br/>记录过滤日志]
    L -->|是| M

    M -->|是| O{HRMS 当前状态}
    M -->|否| P{当前用工类型<br/>是否为 6}

    O -->|人员不存在或已离场| O1[幂等忽略]
    O -->|待入场| O2[删除待入场人员记录]
    O -->|已入场| O3[更新为已离场<br/>写入离场日期]
    O3 --> O4[收口任职记录]
    O4 --> O5[释放项目额度]
    ...
    Q1 --> Q2{新增是否成功}
    Q2 -->|是| Q3[新增完成]
    Q2 -->|否| Q4[并发幂等<br/>更新待入场字段]
    Q -->|是| S{HRMS 当前状态}
    S -->|待入场或未知状态| S1[保持状态<br/>按字段映射更新]
    S -->|已入场且组织未变更| S2[保持已入场<br/>按字段映射更新]
    S -->|已入场且组织变更| S3[保持已入场并更新字段<br/>清空项目和协议关联]
    S3 --> S4[记录组织变更事件<br/>供定时提醒]
    S -->|已离场| S5[改为待入场并更新字段<br/>清空项目和协议关联]

    R -->|待入场| R1[删除待入场人员记录]
    R -->|已入场| R2[更新为已离场<br/>写入离场日期]
    R2 --> R3[收口任职记录]
    R3 --> R4[释放项目额度]
    R -->|已离场| R5[幂等忽略]
    ...
    classDef fromInternal fill:#EDE9FE,stroke:#7C3AED,color:#3B0764,stroke-width:1.5px;
    classDef ignore fill:#DCFCE7,stroke:#16A34A,color:#14532D,stroke-width:1.5px;
    classDef leave fill:#374151,stroke:#111827,color:#FFFFFF,stroke-width:1.5px;
    classDef failure fill:#FEE2E2,stroke:#DC2626,color:#7F1D1D,stroke-width:1.5px;
    class A,B,C,F,H,I,J entry;
    class D,G,K,L,M,O,P,Q,Q2,R,S decision;
    class N,O1,O2,Q3,Q4,S4,S5,R1,R5,Z ignore;
    class O3,O4,O5 leave;
    class Q1,S1,S2,S3 onjob;
    class R2,R3,R4 fromInternal;
    class E failure;
```

> **幂等忽略：**同一类人员消息可能因 MQ 重复投递而被多次收到。若人员已处于目标终态（例如已离场），系统不再重复更新离场状态、收口任职记录或释放项目额度，直接跳过本次业务处理，避免产生重复数据或重复释放额度。

> **提交消费确认：**消费者向 SOFAMQ 返回 `CommitMessage`，表示该消息已处理完毕，MQ 不会再自动投递。当前实现无论处理成功、参数校验失败还是系统异常，都会在记录日志后提交消费确认；因此异常消息不会自动重试，需通过日志监控和人工补偿跟进。

分发顺序固定：先通过 ITO 入口过滤，再优先处理 `personStatus ∈ {1, 2, 5}` 的离职/退休类消息，最后才按当前用工类型分为“当前为 `6`”和“由 `6` 变为其他类型”两条路径。因此，当前用工类型为 `6` 且人员状态为离职/退休类的消息，始终进入离职/退休路径，不会进入在岗路径。
...
图中虚线之后为待入场形成后的后续处理。只要 MQ 人员数据落库成功且状态写为 `-1`，人员即为待入场；无需等待项目、协议关联或提醒任务执行完成。当前消费者在处理异常时记录日志后仍提交消费确认，异常消息不会在该消费者内自动重试。
...
批量关联在同一本地事务内按以下顺序执行；任一校验或写入失败时，人员关联、项目额度流水和任职记录均回滚。

1. 按 `staffIds` 查询人员，校验全部人员存在、均为 `employment_category=6 AND source_from=ITO` 的内包人员，且状态仅为待入场（`-1`）或已入场（`0`）；同时校验人员归属机构位于 `permissionOrgCode` 及其下级机构范围内。
2. 按 `projectCode` 查询目标项目，校验项目状态为生效；读取目标项目业务条线，供后续权限、协议适用范围和额度校验使用。
3. 按 `agreementNo` 筛出生效协议，再按该生效协议编号查询完整协议详情，取得主业务条线和子业务条线。目标项目业务条线命中协议任一适用业务条线时才允许关联；不校验项目与协议是否属于同一机构或是否已预先绑定。
4. 校验当前操作人拥有目标项目业务条线权限。
5. 计算目标项目本次新增占用人数：待入场人员全部计入；已入场人员仅在当前项目与目标项目不同时计入。仅更换协议但项目不变时不重复占用额度。
6. 存在新增占用时，查询目标项目计划额度及历史额度流水，按“项目计划额度 + 历史流水代数和”计算实际剩余数量；不足时整批失败，不执行写入。
7. 逐人组装关联结果：统一写入目标项目、项目业务条线、目标协议和协议乙方供应商。待入场人员直接变更为已入场并写入本次提交日期；已入场人员保持已入场状态。
8. 已入场人员切换项目时，汇总原项目需释放的额度；项目相同但协议不同仅更新协议、供应商和业务条线，不产生项目额度流水；项目和协议均相同视为幂等提交，不更新任何数据。
9. 本批次不存在实际更新人员时直接返回成功；否则写入以下数据：
   - `outsourced_staff_onoffboard_application`：新增一条本次批量关联对应的成功入场申请主单。
   - `outsourced_staff_onoffboard_application_project`：新增目标项目占用及原项目释放的额度流水明细，`request_num` 负数表示占用、正数表示释放。
   - `outsourced_staff`：批量更新人员主表中的项目、协议、供应商、业务条线及待入场人员的入场状态和日期。
   - `outsourced_staff_movement`：为实际发生关联或直接入场的人员新增任职记录。
   - 其中，项目额度本身仅记录在前两张申请及申请项目明细表中。
10. 全部写入成功后返回 `true`；不发起审批流、不调用账号中心，也不发送外包人员变更 MQ。
