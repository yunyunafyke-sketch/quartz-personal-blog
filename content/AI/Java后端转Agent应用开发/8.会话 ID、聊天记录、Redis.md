...
这段配置做了两件事：先创建一个“最多保留 10 条消息、存到 Redis”的 `ChatMemory`，再创建一个已注册 `MessageChatMemoryAdvisor` 的 `ChatClient`。后面的 `chatClient.prompt()` 才能使用会话 ID 读取和保存记忆。
...
public class ChatController {
    ...
    public ChatResponse chat(@Valid @RequestBody ChatRequest request) {
        ...
        if (conversationId == null || conversationId.isBlank()) {
            // 学习示例：实际项目应把 demo-user 替换为登录态中的用户 ID。
...
| `RedisChatMemoryRepository` 找不到 Bean | 未添加 Redis Chat Memory starter，或 Maven 未刷新 | 核对 4.1.3 的依赖并重新加载 Maven。 |
